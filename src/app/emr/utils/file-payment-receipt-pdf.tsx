// File Payment Receipt PDF Generator
// Generates professional PDF receipts for file registration payments

export interface FilePaymentReceiptData {
  receiptId: string;
  patientName: string;
  patientId: string;
  fileNo: string;
  fileType: 'Individual' | 'Family';
  amount: number;
...
export function generateFilePaymentReceiptPDF(data: FilePaymentReceiptData): void {
  ...
  const html = `
    ...
    <head>
      <meta charset="UTF-8">
      <title>File Registration Receipt - ${data.receiptId}</title>
      <style>
        * {
          margin: 0;
          padding: 0;
    ...
    <body>
      ...
      <div class="receipt">
        <div class="header">
          ...
          <div class="receipt-title">File Registration Receipt</div>
          <div class="receipt-id">Receipt No: ${data.receiptId}</div>
          <div class="badge">${data.fileType} File</div>
        </div>
        ...
        <div class="details-section">
          ...
          <div class="details-grid">
            ...
            <div class="detail-item">
              ...
              <div class="detail-value">${data.fileType}</div>
            </div>
            ${data.parentFileId ? `
            <div class="detail-item">
              <div class="detail-label">Parent File ID</div>
              <div class="detail-value">${data.parentFileId}</div>
            </div>
            ` : ''}
          </div>
        </div>
        ...
        <div class="footer">
          ...
          <div class="footer-note">
            <p class="footer-note-text">
              <strong>Important:</strong> This file registration fee is valid for one year from the date of payment. 
              Please present this receipt or your file number for all future visits to the hospital.
              ${data.fileType === 'Family' ? ' This family file covers the main patient and all registered dependents.' : ''}
            </p>
          </div>
          
          <p style="font-size: 11px; color: #9ca3af; margin-top: 20px;">
            Printed on ${new Date().toLocaleString()} | For inquiries: billing@godiyahospital.ng
          </p>
        </div>
      </div>
